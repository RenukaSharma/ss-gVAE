...
root_dir = (os.environ.get("RESULTS_DIR", "./results") + "/syn-dataset/BS-128_baseline_BinClassification")

collated_data =[]

for ratio_l in os.listdir(root_dir):
    # print (ratio_l)    
    for run in os.listdir(os.path.join(root_dir, ratio_l )):
        # print ("The run number is",run)
        if run!="run_6_with-recon-param":
            run_val = float(run[4:])
            ratio_l_val = float(ratio_l[8:])

            file_path = os.path.join(root_dir, ratio_l , run,"results.json")
            if os.path.exists(file_path):
                with open(file_path) as read_file:
                    # print("The results file exists")
                    data = json.load(read_file)
                    auc = data['test_auc']
                    row = [ratio_l_val, run_val, auc]
                    # print("The row is", row)

                    collated_data.append(row)

# print(collated_data)
df = pd.DataFrame(collated_data, columns=["ratio_l", "run","auc"])
# df.to_csv((os.environ.get("RESULTS_DIR", "./results") + "/syn-dataset/binCls_results.csv"))
...
print(df.groupby(["ratio_l"]).mean())
print(df.groupby(["ratio_l"]).std())
