...
res_dir="${RESULTS_DIR}/BS-128/with-SPnoise-no-pollution-inv-loss-noveltest_take-2"
mkdir -p ${res_dir}
                    ...
                    do
                        ...
                        do 
                            ...
                            do                       
                                ...
                                python main.py malaria_dataset cifar10_LeNet ${res_dir}/n_known_outlier_classes_${n_known_outlier_classes}/ratio_l_${ratio_l}/recon_param_${recon_param}/latent_param_${latent_param}/eta_${eta}/val_${val}/baseline_${baseline}/run_${i} ${MALARIA_DATA} --ratio_known_outlier ${ratio_l} --lr 1e-04 --n_epochs 75 --batch_size 128 --weight_decay 0.5e-6 --pretrain False --n_known_outlier_classes ${n_known_outlier_classes} --seed ${i} --recon_param ${recon_param} --latent_param ${latent_param} --eta ${eta} --eps ${val} --tau ${val} --delta ${val} --n_jobs_dataloader 4 --ratio_known_normal 0 --ratio_pollution 0 --known_outlier_class 1 --ablation_type ${baseline} --lr_milestone 10;   # --lr_milestone 40 --lr_milestone 20  --lr_milestone 15
                            done
                        done
                    done
                done 
