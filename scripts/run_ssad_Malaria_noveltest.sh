...
res_dir="${RESULTS_DIR}/results_bayesian_Malaria-patchwise/BS-128/with-SPnoise-no-pollution-inv-loss-noveltest_take-2/baseline-ssad-hybrid-withVarLearning"
mkdir -p ${res_dir}
for i in {0..4}
do
...
res_dir="${RESULTS_DIR}/results_bayesian_Malaria-patchwise/BS-128/with-SPnoise-no-pollution-inv-loss-noveltest_take-2/baseline-ssad"
mkdir -p ${res_dir}
for i in {0..4}
do
    for n_known_outlier_classes in 1
    do
        ...
        do
            ...
            do
                ...
                python baseline_ssad.py malaria_dataset ${res_dir}/ratio_l_${ratio_l}/kappa_${kappa}/run_${i} ${MALARIA_DATA} --ratio_known_outlier ${ratio_l} --kernel rbf --kappa ${kappa} --n_known_outlier_classes ${n_known_outlier_classes} --seed ${i} --ratio_pollution 0 ;
            done
        done
    done      
done
