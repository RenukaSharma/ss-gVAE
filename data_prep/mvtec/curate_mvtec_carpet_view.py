...
dest_dir = (os.environ.get("RESULTS_DIR", "./results") + "/mvtec_AD_dataset/carpet_curated_64")
    	...
    	for j in range(1,20):
    		...
    		if( x2 <= img.size[0] and y2 <= img.size[1]):
    			...
    			if(np.sum(np.array(mask_cropped)/255) >= 2048):			    	
			    	...
			    	img_cropped.save(os.path.join(dest_dir,"no_good_3", defect_type,str(file_wo_ext)+str(i)+"_"+str(j)+".png" ))
			    	mask_cropped.save(os.path.join(dest_dir,"no_good_3", defect_type,"mask",str(file_wo_ext)+str(i)+"_"+str(j)+".png" ))
"""
dest_dir = (os.environ.get("RESULTS_DIR", "./results") + "/mvtec_AD_dataset/carpet_to_use/train/not_good")
# dest_dir = (os.environ.get("RESULTS_DIR", "./results") + "/mvtec_AD_dataset/carpet_to_use/train/good")
...
for fn in os.listdir(dest_dir):
	img1 = Image.open(os.path.join(dest_dir,fn))
	i += 1
	if (i==41):
		break
...
plt.show()
# plt.axis("off")
plt.savefig((os.environ.get("RESULTS_DIR", "./results") + "/mvtec_AD_dataset/fig_not_good.png"))
# plt.savefig((os.environ.get("RESULTS_DIR", "./results") + "/mvtec_AD_dataset/fig_good.png"))
