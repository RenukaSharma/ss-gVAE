...
from Deep_BinClassification import DeepSAD
...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['mnist', 'fmnist', 'cifar10', 'malaria_dataset']))
@click.argument('net_name', type=click.Choice(['mnist_LeNet', 'fmnist_LeNet', 'cifar10_LeNet', 'malaria_net']))
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, eta,
    ...
    deepSAD = DeepSAD(cfg.settings['eta'], cfg.settings['recon_param'], xp_path=xp_path)
    ...
    deepSAD.save_results(export_json=xp_path + '/results_BinClassification.json')
    # deepSAD.save_model(export_model=xp_path + '/model.tar')
    cfg.save_config(export_json=xp_path + '/config_BinClassification.json')
