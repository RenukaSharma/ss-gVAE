...
class DeepSAD(object):
    ...
    def __init__(self, eta: float = 1.0, recon_param: float = 0.5, latent_param: float = 0.5, xp_path: str = "abc"):
    ...
    def train(self, dataset: BaseADDataset, dataset_name: str = 'cifar10',optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 50,
        ...
        if self.ae_net is None:
            ...
            print("$$$$$$$$$$$$ Initialising the ae_net $$$$$$$$$$$$")
        self.optimizer_name = optimizer_name
        self.trainer = DeepSADTrainer(self.c, self.eta, dataset_name= dataset_name ,optimizer_name=optimizer_name, lr=lr, n_epochs=n_epochs,
                                      lr_milestones=lr_milestones, batch_size=batch_size, weight_decay=weight_decay,
                                      device=device, n_jobs_dataloader=n_jobs_dataloader, recon_param = self.recon_param, latent_param= self.latent_param, eps = eps, tau= tau, delta = delta, const = const, xp_path=self.xp_path)
        # Get the model
    ...
    def pretrain(self, dataset: BaseADDataset, dataset_name: str = 'cifar10', optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 100,
                 lr_milestones: tuple = (), batch_size: int = 128, weight_decay: float = 1e-6, device: str = 'cuda',
                 n_jobs_dataloader: int = 0, eps: float = 0.1, tau: float =0.1, delta: float=0.1, xp_path: str ='../'):
        ...
        self.ae_trainer = AETrainer(self.eta,dataset_name= dataset_name ,optimizer_name=optimizer_name, lr=lr, n_epochs=n_epochs, lr_milestones=lr_milestones,
                                    batch_size=batch_size, weight_decay=weight_decay, device=device,
                                    n_jobs_dataloader=n_jobs_dataloader, recon_param = self.recon_param, latent_param= self.latent_param, eps = eps, tau= tau, delta = delta)
    ...
    def load_model(self, model_path, load_ae=False, map_location='cpu'):
        ...
        self.c = model_dict['c']
        print("type(self.net)",type(self.net))
        
        print("type(self.ae_net)",type(self.ae_net))
    ...
    def load_model_bincls(self, model_path, load_ae=False, map_location='cpu'):
        """Load Deep SAD model from model_path."""
