...
class CIFAR10_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class: int = 5, known_outlier_class: int = 3, n_known_outlier_classes: int = 0,
                 ratio_known_normal: float = 0.0, ratio_known_outlier: float = 0.0, ratio_pollution: float = 0.0, length: int=5000):
        super().__init__(root)

        # Define normal and outlier classes
        self.n_classes = 2  # 0: normal, 1: outlier, semi-targets=0 for normal samples, semi-tagerts=-1 for known abnormal and 1 for known normal
...
class MyCIFAR10(CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        if(self.train and random.random()>=0.5):
            ...
            gamma = torch.distributions.gamma.Gamma(1.0/(2.0), 1)                
            y = gamma.rsample()
            p = torch.ones(img.shape)*0.5
            p = p.to('cuda' if torch.cuda.is_available() else 'cpu')
            s = torch.bernoulli(p) - 0.5
            img = np.mean(img) + 2 *(np.std(img)) * s * (y ** (1.0/(2.0)) )
