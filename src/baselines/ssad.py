...
class SSAD(object):
    ...
    def train(self, dataset: BaseADDataset, device: str = 'cpu', n_jobs_dataloader: int = 0, dataset_name='cifar10'):
        ...
        for data in train_loader:
            ...
            if self.hybrid and self.ae_net is None:
                if dataset_name in ('cifar10', 'mvtec', 'malaria_dataset', 'synthetic'):
                    self.ae_net = build_autoencoder("cifar10_LeNet")
                elif dataset_name == 'fmnist':
                    self.ae_net = build_autoencoder("fmnist_LeNet")
                else:  # mnist
                    self.ae_net = build_autoencoder("mnist_LeNet")
            if self.hybrid:
                *_, inputs = self.ae_net(inputs)  # take the 'sample' (last) element as the hybrid feature
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cpu', n_jobs_dataloader: int = 0, dataset_name='cifar10'):
    ...
    def load_ae(self, dataset_name, model_path):
        ...
        elif dataset_name in ('mvtec', 'malaria_dataset', 'synthetic'):
            net_name = 'cifar10_LeNet'
        else:
            raise ValueError(f"Unknown dataset_name for SSAD hybrid AE: {dataset_name}")
